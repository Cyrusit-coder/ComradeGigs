...
# 1. user- admin - all accounts manager
@admin.register(User)
...
# 3. gig or job that admin verifies 
@admin.register(Job)
class JobAdmin(admin.ModelAdmin):
    list_display = ('title', 'client', 'budget', 'status', 'deadline', 'created_at')
    list_filter = ('status', 'created_at')
    search_fields = ('title', 'description', 'client__username')
    list_editable = ('status',) # Allows admin to approve gigs directly from the list
    date_hierarchy = 'created_at'

# 4. applications for admin to see who applied where
...
# 6. skills thus managing  the list of available skills
